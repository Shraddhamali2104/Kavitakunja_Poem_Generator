...
class RetaForm extends StatefulWidget {
  const RetaForm({super.key});

  @override
  State<RetaForm> createState() => _RetaFormState();
}

class _RetaFormState extends State<RetaForm> {
  ...
  Future<void> fetchGrnTypes() async {
    ...
    try {
      ...
      if (grnTypeOptions.isEmpty) {
        // If no GRN types from options controller, create a fallback with default ID 69
        log('No GRN types available from options controller, using fallback');
        setState(() {
          grnTypeList = [
            {'id': 69, 'grn_type': 'Reta-Non Returnable (Default)'},
          ];
          selectedGrnTypeId = 69;
          isLoading = false;
        });
        return;
      }
      ...
      setState(() {
        ...
        // Find Reta type using options controller's helper methods
        Option? gstOption;
        try {
        ...
        if (gstOption != null) {
          ...
          // Use default GRN type ID 69 if GST not found
          selectedGrnTypeId = 69;
          log('No GST type found, using default GRN type ID: 69');
        }
        isLoading = false;
      });
      ...
      // If there's an error, provide fallback with default ID 69
      log('Error in fetchGrnTypes: $e, using fallback');
      setState(() {
        grnTypeList = [
          {'id': 69, 'grn_type': 'GST-Non Returnable (Default)'},
        ];
        selectedGrnTypeId = 69;
        isLoading = false;
      });
            ...
            children: [
              const Center(
                child: Padding(
                  ...
                  child: Text(
                    'This form is for RETA GRN entry.',
                  ...
                  Expanded(
                    child: Column(
                      children: [
                        ...
                        Builder(
                          builder: (context) {
                            // Use options controller directly to get RETA type
                            final grnTypeOptions =
                                optionsController.grnTypeOptions;
                            Option? retaOption;

                            try {
                              retaOption = grnTypeOptions.firstWhere(
                            ...
                            // Update selectedGrnTypeId if RETA option found, otherwise use default 69
                            WidgetsBinding.instance.addPostFrameCallback((_) {
                              if (selectedGrnTypeId !=
                                  (retaOption?.optionId ?? 69)) {
                                setState(() {
                                  selectedGrnTypeId =
                                      retaOption?.optionId ?? 69;
                                });
