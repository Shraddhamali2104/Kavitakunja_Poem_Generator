...
class EstForm extends StatefulWidget {
  const EstForm({super.key});

  @override
  State<EstForm> createState() => _EstFormState();
}

class _EstFormState extends State<EstForm> {
  ...
  WidgetsBinding.instance.addPostFrameCallback((_) async {
    ...
    isEditMode = editingGrnId != null || (args != null && args['isEdit'] == true);
      ...
      // Set default GRN type to EST after options are loaded
      final grnTypeOptions = optionsController.grnTypeOptions;
      Option? estOption;
      try {
        estOption = grnTypeOptions.firstWhere(
          (option) => option.optionName.toLowerCase().contains('est'),
        );
      } catch (e) {
        estOption = null;
      }
      setState(() {
        selectedGrnTypeId = estOption?.optionId ?? 66; // 66 is fallback for EST
      });
  ...
  Future<void> fetchGrnTypes() async {
    ...
    try {
      ...
      if (grnTypeOptions.isEmpty) {
        // If no GRN types from options controller, create a fallback with default ID 66
        log('No GRN types available from options controller, using fallback');
        setState(() {
          grnTypeList = [
            {'id': 66, 'grn_type': 'EST-Non Returnable (Default)'},
          ];
          selectedGrnTypeId = 66;
          isLoading = false;
        });
        return;
      }
      ...
      setState(() {
        ...
        if (gstOption != null) {
          ...
          log(
            'Found EST type from options controller: ID ${gstOption.optionId}, Name: ${gstOption.optionName}',
          );
        } else {
          // Use default GRN type ID 66 if GST not found
          selectedGrnTypeId = 66;
          log('No EST type found, using default GRN type ID: 66');
        }
        isLoading = false;
      });
      ...
      log('Selected EST type ID: $selectedGrnTypeId');
    } catch (e) {
      // If there's an error, provide fallback with default ID 66
      log('Error in fetchGrnTypes: $e, using fallback');
      setState(() {
        grnTypeList = [
          {'id': 66, 'grn_type': 'GST-Non Returnable (Default)'},
        ];
        selectedGrnTypeId = 66;
        isLoading = false;
      });
            ...
            children: [
              const Center(
                child: Padding(
                  ...
                  child: Text(
                    'This form is for EST (Non-GST, Not Returnable) GRN entry.',
                          ...
                          builder: (context) {
                            ...
                            WidgetsBinding.instance.addPostFrameCallback((_) {
                              if (selectedGrnTypeId !=
                                  (estOption?.optionId ?? 66)) {
                                setState(() {
                                  selectedGrnTypeId = estOption?.optionId ?? 66;
                                });
                              }
                            });
