  ...
  Future<void> fetchGrnTypes() async {
    ...
    try {
      ...
      if (grnTypeOptions.isEmpty) {
        // If no GRN types from options controller, create a fallback with default ID 70
        log('No GRN types available from options controller, using fallback');
        setState(() {
          grnTypeList = [
            {'id': 70, 'grn_type': 'RETR-Non Returnable (Default)'},
          ];
          selectedGrnTypeId = 70;
          isLoading = false;
        });
        return;
      }
      ...
      setState(() {
        ...
        // Find RETR type using options controller's helper methods
        Option? gstOption;
        try {
        ...
        if (gstOption != null) {
          ...
          // Use default GRN type ID 70 if GST not found
          selectedGrnTypeId = 70;
          log('No GST type found, using default GRN type ID: 70');
        }
        isLoading = false;
      });
      ...
      // If there's an error, provide fallback with default ID 70
      log('Error in fetchGrnTypes: $e, using fallback');
      setState(() {
        grnTypeList = [
          {'id': 70, 'grn_type': 'GST-Non Returnable (Default)'},
        ];
        selectedGrnTypeId = 70;
        isLoading = false;
      });
      ...
      if (grnResponse.statusCode == 200 || grnResponse.statusCode == 201) {
        ...
        if (grnId != null && itemEntries.isNotEmpty) {
          ...
          if (itemsResponse.statusCode == 200 ||
            ...
            // GRN created/updated but items failed
            setState(() {
              isLoading = false;
              isSubmitting = false;
            });
            ...
            children: [
              const Center(
                child: Padding(
                  ...
                  child: Text(
                    'This form is for RETR GRN entry.',
                  ...
                  Expanded(
                    child: Column(
                      children: [
                        ...
                        Builder(
                          builder: (context) {
                            // Use options controller directly to get RETR type
                            final grnTypeOptions =
                                optionsController.grnTypeOptions;
                            Option? retrOption;

                            try {
                              retrOption = grnTypeOptions.firstWhere(
                            ...
                            // Update selectedGrnTypeId if retr option found, otherwise use default 70
                            WidgetsBinding.instance.addPostFrameCallback((_) {
                              if (selectedGrnTypeId !=
                                  (retrOption?.optionId ?? 70)) {
                                setState(() {
                                  selectedGrnTypeId =
                                      retrOption?.optionId ?? 70;
                                });
