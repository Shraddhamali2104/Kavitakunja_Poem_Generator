...
class GstForm extends StatefulWidget {
  const GstForm({super.key});

  @override
  State<GstForm> createState() => _GstFormState();
}
...
class _GstFormState extends State<GstForm> {
  ...
  void initState() {
    ...
    // Detect edit mode from route param or arguments
  ...
  Future<void> fetchGrnTypes() async {
    ...
    try {
      ...
      if (grnTypeOptions.isEmpty) {
        // If no GRN types from options controller, create a fallback with default ID 68
        log('No GRN types available from options controller, using fallback');
        setState(() {
          grnTypeList = [
            {'id': 68, 'grn_type': 'GST-Non Returnable (Default)'},
          ];
          selectedGrnTypeId = 68;
          isLoading = false;
        });
        return;
      }
      ...
      setState(() {
        ...
        // Find GST type using options controller's helper methods
        Option? gstOption;
        try {
        ...
        if (gstOption != null) {
          ...
          // Use default GRN type ID 68 if GST not found
          selectedGrnTypeId = 68;
          log('No GST type found, using default GRN type ID: 68');
        }
        isLoading = false;
      });
      ...
      // If there's an error, provide fallback with default ID 68
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      if (grnResponse.statusCode == 200 || grnResponse.statusCode == 201) {
        ...
        grnId = isEditMode && editingGrnId != null
            ? editingGrnId
            : grnResponseData['grnId'];
        grnMessage = grnResponseData['message'] ??
            (isEditMode ? 'GRN updated successfully' : 'GRN created successfully');
        ...
        if (grnId != null && itemEntries.isNotEmpty) {
          ...
          // For edit mode, send PUT including existing item ids to update; for create, POST
          ...
          if (itemsResponse.statusCode == 200 ||
            ...
            try {
              ...
              SnackbarUtils.showError(
                '${isEditMode ? 'GRN updated' : 'GRN created'} successfully (ID: $grnId), but failed to ${isEditMode ? 'replace' : 'add'} items: $errorMessage',
              );
            } catch (parseError) {
              SnackbarUtils.showError(
                '${isEditMode ? 'GRN updated' : 'GRN created'} successfully (ID: $grnId), but failed to ${isEditMode ? 'replace' : 'add'} items: ${itemsResponse.body}',
              );
            }
          }
        } else {
  ...
  Future<void> _loadGrnForEdit(int grnId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // month_of_invoice is numeric; convert to Month, Year is not trivial without year; keep existing input
            ...
            children: [
              const Center(
                child: Padding(
                  ...
                  child: Text(
                    'This form is for GST-Non Returnable GRN entry.',
                  ...
                  Expanded(
                    child: Column(
                      children: [
                        ...
                        Builder(
                          builder: (context) {
                            // Use options controller directly to get GST type
                            ...
                            // Update selectedGrnTypeId if GST option found, otherwise use default 68
                            if (gstOption != null) {
                              WidgetsBinding.instance.addPostFrameCallback((_) {
                                if (selectedGrnTypeId != gstOption!.optionId) {
                                  setState(() {
                                    selectedGrnTypeId = gstOption!.optionId;
                                  });
                                }
                              ...
                              // Set default GRN type ID to 68 if no GST option found
                              WidgetsBinding.instance.addPostFrameCallback((_) {
                                if (selectedGrnTypeId != 68) {
                                  setState(() {
                                    selectedGrnTypeId = 68;
                                  });
                                }
